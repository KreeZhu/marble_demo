...
test('shot mechanic check accepts only mechanics the player actually used', () => {
  const result = shotMeetsRequiredMechanics({
    requiredMechanics: ['wallBounce', 'portal', 'movingGate', 'boostBounce', 'relay', 'switchDoor', 'launcherReturn'],
    events: {
      wallBounces: 1,
      ...
      boostBounces: new Set(['boost-wall']),
      launcherReturns: 1,
...
test('shot mechanic check reports missing active mechanics', () => {
  ...
  assert.deepEqual(result.missing, ['墙面反弹', '传送门', '移动障碍时机', '绿色弹力墙', '中继发射器', '开关门', '回到 A 点再发射']);
});
